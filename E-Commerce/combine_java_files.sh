...
# 프로젝트 내 모든 Java 파일 결합
find "$PROJECT_DIR" -name "*.java" | while read -r file; do
...
for file in "$PROJECT_DIR/build.gradle" "$PROJECT_DIR/settings.gradle" \
            ...
            $(find "$PROJECT_DIR" -name "Dockerfile") \
            $(find "$PROJECT_DIR" -name "docker-compose.yml") \
            $(find "$PROJECT_DIR" -name "*.properties") \
            $(find "$PROJECT_DIR" -name "*.yml"); do
    if [ -f "$file" ];then
        echo "File: $file" >> "$OUTPUT_FILE"
        echo "--------" >> "$OUTPUT_FILE"
        cat "$file" >> "$OUTPUT_FILE"
